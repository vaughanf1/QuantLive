...
<task type="auto">
  ...
  <action>
    1. Create `app/api/chart.py` with an APIRouter (prefix="/chart", tags=["chart"]).

    2. **GET /chart/candles endpoint:**
       ```python
       @router.get("/candles")
    ...
    3. **GET /chart/signals endpoint:**
       ...
       - For each signal, compute:
         - `time`: Unix seconds from signal.created_at (int)
    ...
    4. **GET /chart endpoint (HTML page):**
       ```python
    ...
    6. **Create templates directory:** Ensure `app/templates/` directory exists (create it if needed, even if chart.html is created in Task 2).
