...
must_haves:
  truths:
    ...
    - "TelegramNotifier.format_outcome() produces HTML-formatted message with result type, entry, exit price, P&L pips, duration"
  ...
  artifacts:
    ...
    - path: "requirements.txt"
      provides: "jinja2 dependency added"
      contains: "jinja2"
  key_links:
...
Output: A working TelegramNotifier class integrated into the signal pipeline that sends formatted messages on each new signal. Outcome notification capability is built and ready for Phase 6 wiring.
</objective>
...
<task type="auto">
  ...
  <action>
    1. Add `jinja2>=3.1.0` to requirements.txt (needed for Plan 05-02 chart template, add it now so both plans don't conflict on the same file).

    2. Add two optional settings to `app/config.py` Settings class:
       - `telegram_bot_token: str = ""` -- empty string default means disabled
       - `telegram_chat_id: str = ""` -- empty string default means disabled

    3. Create `app/services/telegram_notifier.py` with class `TelegramNotifier`:
  ...
  <verify>
    ...
    Verify requirements.txt contains jinja2.
  </verify>
  <done>
  ...
  <action>
    ...
    1. After `signals = await pipeline.run(session)`, add notification logic:
       ...
       if signals:
           ...
           if notifier.enabled:
               # Look up strategy name for message formatting
               strategy_name_for_msg = strategy_name if 'strategy_name' in dir() else "Unknown"
               for sig in signals:
                   await notifier.notify_signal(sig, strategy_name=strategy_name_for_msg)
       ```

    2. **Important:** The `strategy_name` variable is NOT in scope inside `run_signal_scanner()` since `pipeline.run()` handles selection internally. To get the strategy name:
       - Option A (simple): Query the Strategy table using signal.strategy_id after pipeline returns. This is one extra query but clean.
       - Option B: Have pipeline.run() return strategy_name alongside signals.

       **Use Option A:** After pipeline.run() returns signals, for each signal, do a lightweight query:
       ```python
    ...
    3. The notification block must be INSIDE the try/except already wrapping run_signal_scanner, so any notification errors are caught by the existing handler. But also, notify_signal itself never raises. This is double-safety.

    4. Do NOT modify signal_pipeline.py -- keep the notifier wiring in jobs.py (the job handler layer), not the pipeline itself. This keeps the pipeline pure (DB operations only) and notifications as a post-persist side effect.
  </action>
  <verify>
...
5. `grep -c 'jinja2' requirements.txt` returns 1
6. All existing tests still pass: `PYTHONPATH=. python -m pytest tests/ -x -q`
</verification>
